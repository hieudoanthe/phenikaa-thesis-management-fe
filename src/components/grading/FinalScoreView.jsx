...
const FinalScoreView = ({ topicId }) => {
  ...
  const getStatusLabel = (status) => {
    switch (status) {
      ...
      case 'PENDING': return 'Chờ chấm điểm';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(finalScore.status)}`}>
              ...
              <div key={evaluation.evaluationId} className="bg-white border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <h5 className="font-medium text-gray-900">
                      {evaluation.evaluationType === 'SUPERVISOR' && 'Giảng viên hướng dẫn'}
                      {evaluation.evaluationType === 'REVIEWER' && 'Giảng viên phản biện'}
                      {evaluation.evaluationType === 'COMMITTEE' && 'Hội đồng chấm'}
                    </h5>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
                  <div>
                    ...
                    <span className="ml-1 font-medium">{formatScore(evaluation.contentScore)}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Thuyết trình:</span>
                    <span className="ml-1 font-medium">{formatScore(evaluation.presentationScore)}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Kỹ thuật:</span>
                    <span className="ml-1 font-medium">{formatScore(evaluation.technicalScore)}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Sáng tạo:</span>
                    <span className="ml-1 font-medium">{formatScore(evaluation.innovationScore)}</span>
                  </div>
                  {evaluation.defenseScore && (
                    <div>
                      <span className="text-gray-600">Bảo vệ:</span>
                      <span className="ml-1 font-medium">{formatScore(evaluation.defenseScore)}</span>
                    </div>
                  )}
                </div>
                ...
                {evaluation.comments && (
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    ...
                    <p className="text-sm text-gray-900 mt-1">{evaluation.comments}</p>
                  </div>
                )}
              </div>
            ))}
